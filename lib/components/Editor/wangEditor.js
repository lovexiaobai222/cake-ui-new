...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof this.define === 'function' && this.define.amd ? this.define(factory) : global.wangEditor = factory();
})(void 0, function () {
  /*
      富文本编辑器  默认配置信息
  */
  var config = {
    ...
    downFileServer: '',
    // 上传文件的自定义参数
    uploadFileParams: {// token: 'abcdef12345'
    },
    // 上传文件的自定义header
    uploadFileHeaders: {// 'Accept': 'text/x-json'
    },
    // 自定义上传文件超时时间 ms
    ...
    uploadFileHooks: {
      ...
      success: function success(xhr, editor, result) {// 上传并返回结果，文件插入成功之后触发
      },
      fail: function fail(xhr, editor, result) {// 上传并返回结果，但文件插入错误时触发
      },
      error: function error(xhr, editor) {// 文件上传出错时触发
      },
      timeout: function timeout(xhr, editor) {// 文件上传超时时触发
      }
    }
  };
  /*
    ...
    if (typeof Object.assign !== 'function') {
      Object.assign = function (target, varArgs) {
        ...
        for (var _index = 1; _index < arguments.length; _index++) {
          var nextSource = arguments[_index];
  ...
  */
  ...
  function FontSize(editor) {
    ...
    this.droplist = new DropList(this, {
      ...
      // droplist 以列表形式展示
      list: [{
        $elem: $('<span style="font-size: 10px;">10px</span>'),
        value: '1'
      }, {
  ...
  */
  ...
  function getPasteHtml(e, filterStyle, ignoreImg) {
    ...
    console.log(pasteHtml, "粘贴的html"); // 过滤word中状态过来的无用字符
  ...
  Text.prototype = {
    ...
    _pasteHandle: function _pasteHandle() {
      ...
      $textElem.on('paste', function (e) {
        ...
        e.preventDefault(); // 粘贴图片和文本，只能同时使用一个
  ...
  */
  ...
  UploadImg.prototype = {
    ...
    uploadImg: function uploadImg(files) {
      ...
      var downImgServer = config.downImgServer;
      var uploadImgShowBase64 = config.uploadImgShowBase64 || true; //图片是否显示 base64 格式

      var maxSize = config.uploadImgMaxSize; //图片最大尺寸

      var maxSizeM = maxSize / 1024 / 1024;
      var maxLength = config.uploadImgMaxLength || 10000; //图片最多上传张数

      var uploadImgName = config.uploadImgName || ''; //上传文件名

      var uploadImgParams = config.uploadImgParams || {}; //上传图片参数

      var uploadImgParamsWithUrl = config.uploadImgParamsWithUrl; //上传图片参数是否需拼接到url（false）

      var uploadImgHeaders = config.uploadImgHeaders || {}; //上传图片请求头

      var hooks = config.uploadImgHooks || {}; //上传图片 hook : before  success  fail  error  timeout

      var timeout = config.uploadImgTimeout || 3000; //上传图片超时

      var withCredentials = config.withCredentials; //上传图片是否携带cookie信息

      if (withCredentials == null) {
        withCredentials = false;
      }

      var customUploadImg = config.customUploadImg; //自定义上传图片
      ...
      arrForEach(resultFiles, function (file) {
        var name = uploadImgName || file.name;
      ...
      if (uploadImgServer && typeof uploadImgServer === 'string') {
        ...
        xhr.onreadystatechange = function () {
          ...
          if (xhr.readyState === 4) {
            ...
            result = xhr.responseText; // 上传图片 hook - success

            if (hooks.success && typeof hooks.success === 'function') {
              hooks.success(xhr, editor, result); //     if(result.url){
  ...
  */
  ...
  UploadFile.prototype = {
    ...
    uploadFile: function uploadFile(files) {
      ...
      var _this3 = this; // ------------------------------ 获取配置信息 上传文件------------------------------


      var editor = this.editor;
      var config = editor.config;
      var uploadFileServer = config.uploadFileServer;
      var downFileServer = config.downFileServer;
      var maxSize = config.uploadFileMaxSize; //文件最大尺寸

      var maxSizeM = maxSize / 1024 / 1024;
      var uploadFileParams = config.uploadFileParams || {}; //上传文件参数

      var uploadFileParamsWithUrl = config.uploadFileParamsWithUrl; //上传文件参数是否需拼接到url（false）

      var uploadFileHeaders = config.uploadFileHeaders || {}; //上传文件请求头

      var hooks = config.uploadFileHooks || {}; //上传文件 hook : before  success  fail  error  timeout

      var timeout = config.uploadFileTimeout || 10000; //上传文件超时

      var withCredentials = config.withCredentials; //上传文件是否携带cookie信息

      if (withCredentials == null) {
        withCredentials = false;
      }

      var customUploadFile = config.customUploadFile; //自定义上传文件
      // ------------------------------ 验证文件信息 ------------------------------
      ...
      if (customUploadFile && typeof customUploadFile === 'function') {
        customUploadFile(resultFiles, this.insertLinkImg.bind(this)); // 阻止以下代码执行
      ...
      if (uploadFileServer && typeof uploadFileServer === 'string') {
        // 添加参数
        var uploadFileServerArr = uploadFileServer.split('#');
        uploadFileServer = uploadFileServerArr[0];
        var uploadFileServerHash = uploadFileServerArr[1] || '';
        objForEach(uploadFileParams, function (key, val) {
          // 因使用者反应，自定义参数不能默认 encode ，由 v3.1.1 版本开始注释掉
          // val = encodeURIComponent(val)
          // 第一，将参数拼接到 url 中
        ...
        xhr.onreadystatechange = function () {
          ...
          if (xhr.readyState === 4) {
            ...
            result = xhr.responseText; // 上传文件 hook - success
        ...
        objForEach(uploadFileHeaders, function (key, val) {
          xhr.setRequestHeader(key, val);
        }); // 跨域传 cookie
  ...
  */
  ...
  Editor.prototype = {
    ...
    create: function create() {
      ...
      this._initUploadImg(); // 添加 文件上传


      this._initUploadFile(); // 初始化选区，将光标定位到内容尾部
